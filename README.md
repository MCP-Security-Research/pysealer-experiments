...
`run_experiments.py` executes the following steps in order:

| Step | Command | Description |
|------|---------|-------------|
| 1 | `pysealer init` | Initialises a `.pysealer` config in each tool directory |
| 2 | `pysealer lock <file>` | Seals the current state of all `@mcp.tool()` decorators |
| 3 | `pysealer check <file>` | Baseline check — both files should pass cleanly |
| 4 | `execute_tool_poisoning_attack.py` | Mutates `tool_poisoning.py` in-place with the poisoned payload |
| 4 | `execute_tool_shadowing_attack.py` | Injects the shadow tool into `tool_shadowing.py` |
| 5 | `pysealer check <file>` | Post-attack check — both files should now fail with diffs |
| 6 | Cleanup | Removes any side-effects (hooks, env vars) |
